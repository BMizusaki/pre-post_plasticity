...
for nrun in range(nruns):
    ...
    if nrun > init: #to look at interval before plasticity
        Ap = 0.05
        Ad = -Ap*1.05
    else:
        Ad=0.
    ...
    if Mf.num_spikes>0:
        ...
        #    for k in range(1,Ms.num_spikes):
             with open('output_lat_sto_duration','a') as nfile:
                nfile.write("%f %d \n" % ((Mf.t[Mf.num_spikes-1]-Mf.t[0])/ms,nrun))
             with open('output_lat_sto_frequency','a') as nfile:
...
###to print latency
with open('output_lat_sto','a') as nfile:
    for k in range(nruns):
        nfile.write("%f %d\n" % (cicle[k]-T[k]-cicle[0],k))
